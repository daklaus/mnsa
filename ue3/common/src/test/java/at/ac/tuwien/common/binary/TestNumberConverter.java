...
public class TestNumberConverter {
	...
	public void testHexToBytesShouldPass() {
		...
		assertArrayEquals("Single byte doesn't work",
				new byte[] { (byte) 0x42 }, NumberConverter.hexStringToBytes("42"));
		assertArrayEquals("Single character doesn't work",
				new byte[] { (byte) 0x02 }, NumberConverter.hexStringToBytes("2"));
		assertArrayEquals("Odd string length doesn't work",
				NumberConverter.hexStringToBytes("042"), new byte[] { (byte) 0x00,
						(byte) 0x42 });
		assertArrayEquals("Lowercase hex-letters don't work",
				new byte[] { (byte) 0x3f }, NumberConverter.hexStringToBytes("3f"));
		assertArrayEquals("Uppercase hex-letters don't work",
				new byte[] { (byte) 0x3f }, NumberConverter.hexStringToBytes("0x3F"));
		...
		assertArrayEquals("Tabs and newlines inbetween don't work", new byte[] {
				...
				NumberConverter.hexStringToBytes(" 5 a e    6 \t  7  \n  8   h  "));
		assertArrayEquals(
				...
				NumberConverter.hexStringToBytes("46_5F_13_28 a4_63 18_e9_6B 13"));
	}
